function [dec_mat, y_pred_self, auc_self, y_pred_self_multi] = ...
    f_pred_each_cluster_using_self_forWhole(X, y, clus, dec_mat, n_clusters,...
...
for j=1:n_clusters
    ...
    if length(yj) > 15
        if useParfor == 1
            [y_pred, auc, ~] = f_weka_RF_arff_k_fo_3_parfor(Xj, yj, 10);
        else
...
% auc_self = f_my_01_kappa(y_pred_self_noNan, y_tmp);
