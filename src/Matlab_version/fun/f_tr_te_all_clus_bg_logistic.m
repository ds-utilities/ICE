function [ y_pred_all ] = f_tr_te_all_clus_bg_logistic(X_tr, y_tr, X_te, y_te, clus, doNorm)
...
y_pred_all(:, 1) = f_weka_bg_logistic_tr_te(X_tr, y_tr, X_te, y_te);

% start from the second col, the result is for each cluster
for i=1:length(clus)
    y_pred_te = f_weka_bg_logistic_tr_te(X_tr(clus{i}, :), y_tr(clus{i}), X_te, y_te);
