function [dec_mat] = f_AB_dec_tab(X, y, clus)
% Revised from function f_AB_vali_1_data(). 
% This version is simple than the original method that it uses as
%  many instances as possible. For example, when using a bigger cluster 
%  to predict a smaller cluster, previously, it only use a portion of
%  the bigger cluster to let the training data size equal to the smaller
%  cluster, however, this version will use as many instance as possible. 
%  Using whole to predict each cluster is the same as before; 
%  A predict B will use all data. I am not 100% sure, but I
%  believe that when use B to predict A, I should keep the previous setup
%  that use all B and part A and do cross-validation. 
% Now I am sure that when using B to predict A, use a simple B or B+C to
%  predict A rather than using 10-fold. Just keep simple. And thus, there
%  would be a high chance that the tactic algorithm will always pick either
%  whole data or cluster A; if B is really good, it will pickup B. This
%  setup can almost guarantee some improvement from using 'whole' or 'self' 
% When do self prediction, use 10-fold. 
...
dec_mat = nan(n_clusters + 1, n_clusters);
    ...
    for j=1:n_clusters
        ...
        if length(yj) > 20
            [y_pred, auc] = f_weka_RF_arff_10_fo(Xj, yj);
            dec_mat(j, j) = auc;
        end
    end
    ...
    %% --------------------------------------------------------------------
    % Using larger clusters to predict smaller clusters (A predicts B)
    ...
    %% --------------------------------------------------------------------
    % Using smaller clusters to predict larger clusters (B predicts A)
    try
    for r = 2:n_clusters
        for c = 1 : r - 1
            %r,c,
