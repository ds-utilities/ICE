function [val_mat, val_mat_kap] = f_AB_vali_1_data_loo(X, y, clus, ...
...
%     try
    for j=1:n_clusters
        ...
        if length(yj) > 15
            if useParfor == 1
                [y_pred, auc, kap] = f_weka_RF_arff_k_fo_3_parfor(Xj, yj, length(yj));
            else
                [y_pred, auc, kap] = f_weka_RF_arff_k_fo_3(Xj, yj, length(yj));
            end
            val_mat(j, j) = auc;
            val_mat_kap(j, j) = kap;
        end
        ...
        for c = 1 : r - 1
            ...
            if length(yb) > 20
                if length(yb) < length(ya)
                    [y_pred, auc, kap] = f_weka_RF_arff_loo_2_parfor(Xa, ya,...
                        Xb, yb);
                else
                    [y_pred, auc, kap] = f_weka_RF_tr_te(Xb, yb, Xa, ya);
                end
...
%     try
    if useParfor == 1
    [y_pred_whole, auc_whole, kap_whole] = f_weka_RF_arff_k_fo_3_parfor(X, y, length(y));
    else
    [y_pred_whole, auc_whole, kap_whole] = f_weka_RF_arff_k_fo_3(X, y, length(y));
    end
