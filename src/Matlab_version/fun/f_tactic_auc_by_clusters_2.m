function [auc_whole, auc_self, auc_other, auc_tactic, ...
    ...
    f_tactic_auc_by_clusters_2(X, y, result, adv_whole, adv_self)
...
for c = 1:n_clusters
    ...
    elseif max(tmp) == tmp(c)
        ...
        target = y_pred_self(clus{c});
        y_pred_tactic(clus{c}) = f_my_quantileNorm(templete, target);
