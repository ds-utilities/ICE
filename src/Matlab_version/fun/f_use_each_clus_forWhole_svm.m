function [y_pred_multi] = ...
    f_use_each_clus_forWhole_svm(X, y, clus, y_pred_whole, useParfor,op)
...
for j=1:n_clusters
    ...
    if length(unique(yj)) == 1
        ...
        if length(yj) >=20  %> 15
            % ------------------ for self ------------------
            %if useParfor == 1
                [y_pred, auc, ~] = f_weka_svm2_arff_k_fo_3_parfor(Xj, yj, 10, op);
            %else
            ...
            y_other = y( ix_other ) ;
            % weka prediction
            y_pred = f_weka_svm2_tr_te(Xj, yj, X_other, y_other, op);
            % Normalization
