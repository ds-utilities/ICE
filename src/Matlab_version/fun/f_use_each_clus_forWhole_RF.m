function [y_pred_multi] = ...
    f_use_each_clus_forWhole_RF(X, y, clus, y_pred_whole, useParfor, I)
...
for j=1:n_clusters
    ...
    if length(unique(yj)) == 1
        y_pred_multi(:, j) = unique(yj);
    else
        if length(yj) >= 20
            % ------------------ for self ------------------
            if useParfor == 1
                [y_pred, auc, ~] = f_weka_RF_arff_k_fo_3_parfor(Xj, yj, 10, I);
            else
                [y_pred, auc, ~] = f_weka_RF_arff_k_fo_3(Xj, yj, 10, I);
            end        
            % Normalization
            ...
            y_other = y( ix_other ) ;
            % weka prediction
            y_pred = f_weka_RF_tr_te(Xj, yj, X_other, y_other, I);
            % Normalization
