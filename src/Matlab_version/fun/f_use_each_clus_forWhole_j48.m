function [y_pred_multi] = ...
    f_use_each_clus_forWhole_j48(X, y, clus, y_pred_whole, useParfor)
...
for j=1:n_clusters
    ...
    if length(yj) > 15
        % ------------------ for self ------------------
        %if useParfor == 1
            [y_pred, auc, ~] = f_weka_j48_arff_k_fo_3_parfor(Xj, yj, 10);
        %else
        ...
        y_other = y( ix_other ) ;
        % weka prediction
        y_pred = f_weka_j48_tr_te(Xj, yj, X_other, y_other);
        % Normalization
