function [val_mat, val_mat_kap] = f_AB_vali_1_data_bgsvm(X, y, clus, n_clusters)
% 
...
val_mat_kap = nan(n_clusters + 1, n_clusters);
    ...
    for j=1:n_clusters
        ...
        if length(yj) > 20
            ...
            [y_pred, auc, kap] = f_weka_bg_svm_arff_k_fo_3_parfor(Xj, yj, 10);
            val_mat(j, j) = auc;
            val_mat_kap(j, j) = kap;
        end
    end
    ...
    for r = 1:n_clusters
        for c = r + 1 : n_clusters
            ...
            [y_pred, auc, kap] = f_weka_bg_svm_tr_te(X_tr, y_tr, X_te, y_te);
            val_mat(r, c) = auc;
            val_mat_kap(r, c) = kap;
            %end
        end
        ...
        for c = 1 : r - 1
            ...
            if length(yb) > 20
                if length(yb) < length(ya)
                    [y_pred, auc, kap] = f_weka_bg_svm_arff_10_fo_2(Xa, ya,...
                        Xb, yb);
                else
                    [y_pred, auc, kap] = f_weka_bg_svm_tr_te(Xb, yb, Xa, ya);
                end
    ...
    [y_pred, ~] = f_weka_bg_svm_arff_k_fo_3_parfor(X, y);
    for j=1:n_clusters
